...
const getCurrentDateTime = () => dayjs(new Date()).format("YYYY-MM-DDTHH:mm");
...
export default function InstallmentsModal({ transaction, onClose }) {
  ...
  const handleAddPayment = async (e) => {
    ...
    // Convert the local input string (YYYY-MM-DDTHH:mm) into a UTC ISO string (Z)
    const localTimeStr = form.dateTransact;
    // Use dayjs() to parse the local string *in the client's local timezone*
    const utcISOString = dayjs(localTimeStr).toISOString();
  ...
  return (
    <div className="fixed inset-0 z-[99999] bg-black/60 backdrop-blur-sm flex justify-center items-center">
      <div className=" bg-white w-full max-w-lg rounded-2xl border border-gray-700 shadow-2xl overflow-hidden h-[80vh]">
        {/* Header */}
